...
class URLFactory
{
    ...
    */
    ...
    class func searchByCity(city: String) -> NSURL
    {
        return ccSearchUrl("concerts/search", args: ["city" : city])
    }
    ...
    private class func ccSearchUrl(name: String, var args: [String: String]) -> NSURL
    {
        let
        baseURL      = "http://cc-server:5000/cc/api/v1.0/",
        queryString  = queryWithArgs(args),
        absolutePath = baseURL + name + "?" + queryString
        return NSURL(string: absolutePath)!
    }
